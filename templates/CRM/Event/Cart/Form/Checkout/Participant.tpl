  ...
  <fieldset class="participant" id="event_{$event_id}_participant_{$participant_id}">
    <div class="clearfix participant-inner">
      <h4>
...
onclick="delete_participant({$event_id}, {$participant_id}); return false;">{ts}(-) Delete{/ts} {$form->name}</a>
      <input type="hidden" name="event[{$event_id}][participant][{$participant_id}]" value="{$participant_id}">
    </div>
